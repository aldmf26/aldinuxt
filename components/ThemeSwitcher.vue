<template>
  <div class="fixed top-6 left-6 z-[100] flex gap-2 p-1.5 bg-bg/80 backdrop-blur-md rounded-full border border-primary/10 shadow-2xl transition-all duration-500">
    <button 
      ...
      :class="[
        ...
        currentTheme === theme.class ? 'border-primary scale-110' : 'border-transparent opacity-60 hover:opacity-100'
      ]"
...
const themes = [
  ...
  { name: 'Dark', class: 'theme-dark', color: '#0d1117' },
  { name: 'Luxury', class: 'theme-luxury', color: '#D4AF37' },
  { name: 'Synthwave', class: 'theme-synthwave', color: '#f62d8f' }
]
